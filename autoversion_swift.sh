...
#   Copyright 2015-2017 Malcolm Teas.  See https://github.com/malcolmteas/AutoVersionXcode
...
#   Put it all together
fullversion="${version} (${buildNumber}) ${buildHash}"
...
echo "//  Application version information automatically generated at build time." > "${PROJECT_DIR}/version.swift"
echo "//  Do not commit this file to your code repository." >> "${PROJECT_DIR}/version.swift"
echo "//  This is generated by a script in Run Script build phase." >> "${PROJECT_DIR}/version.swift"
echo "//  See script for details on updating version and copyright." >> "${PROJECT_DIR}/version.swift"
        
echo >> "${PROJECT_DIR}/version.swift"
echo "//  Application information" >> "${PROJECT_DIR}/version.swift"
...
echo "let CopyrightYear = \"${COPYRIGHT_YEAR}\"" >> "${PROJECT_DIR}/version.swift"
echo "let Copyright = \"Copyright ${COPYRIGHT_YEAR} by ${COPYRIGHT_NAME}. All rights reserved.\"" >> "${PROJECT_DIR}/version.swift"
        
echo >> "${PROJECT_DIR}/version.swift"
echo "//  Version information" >> "${PROJECT_DIR}/version.swift"
echo "let FullVersion = \"${fullversion}\"" >> "${PROJECT_DIR}/version.swift"
echo "let Version = \"${version}\"    // From Summary tab or short bundle string in plist file" >> "${PROJECT_DIR}/version.swift"
echo "let SimpleVersion = \"${simpleVersion}\"" >> "${PROJECT_DIR}/version.swift"
        
echo >> "${PROJECT_DIR}/version.swift"
echo "//  Information about this build" >> "${PROJECT_DIR}/version.swift"
echo "let BuildNumber = \"${buildNumber}\"    // Increment this by changing CFBundleVersion in app plist." >> "${PROJECT_DIR}/version.swift"
echo "let BuildHash = \"${buildHash}\"        // git commit hash (dirty if not checked in)" >> "${PROJECT_DIR}/version.swift"
echo "let BuildDate = \"`date -u`\"" >> "${PROJECT_DIR}/version.swift"
        
echo >> "${PROJECT_DIR}/version.swift"
echo "//  Environment version" >> "${PROJECT_DIR}/version.swift"
echo "let XCODE_VERSION = \"${XCODE_VERSION_ACTUAL}\"" >> "${PROJECT_DIR}/version.swift"
echo "let SDK_NAME = \"${SDK_NAME}\"" >> "${PROJECT_DIR}/version.swift"
echo "let DEPLOYMENT_TARGET = \"${IPHONEOS_DEPLOYMENT_TARGET}\"" >> "${PROJECT_DIR}/version.swift"
        
echo >> "${PROJECT_DIR}/version.swift"
echo "//  Release phase" >> "${PROJECT_DIR}/version.swift"
echo " enum ReleasePhaseEnum {" >> "${PROJECT_DIR}/version.swift"
echo "    case Dev, Alpha, Beta, Final, Release" >> "${PROJECT_DIR}/version.swift"
echo " }" >> "${PROJECT_DIR}/version.swift"
echo >> "${PROJECT_DIR}/version.swift"
        
if [[ "$phaseLetter" == "D" ]]
then
        echo "var ReleasePhase: ReleasePhaseEnum = .Dev" >> "${PROJECT_DIR}/version.swift"
        echo "let PhaseNumber = ${phaseNumber}" >> "${PROJECT_DIR}/version.swift"
fi
if [[ "$phaseLetter" == "A" ]]
then
        echo "var ReleasePhase: ReleasePhaseEnum = .Alpha" >> "${PROJECT_DIR}/version.swift"
        echo "let PhaseNumber = ${phaseNumber}" >> "${PROJECT_DIR}/version.swift"
fi
if [[ "$phaseLetter" == "B" ]]
then
        echo "var ReleasePhase: ReleasePhaseEnum = .Beta" >> "${PROJECT_DIR}/version.swift"
        echo "let PhaseNumber = ${phaseNumber}" >> "${PROJECT_DIR}/version.swift"
fi
if [[ "$phaseLetter" == "F" ]]
then
        echo "var ReleasePhase: ReleasePhaseEnum = .Final" >> "${PROJECT_DIR}/version.swift"
        echo "let PhaseNumber = ${phaseNumber}" >> "${PROJECT_DIR}/version.swift"
fi
if  [[ "$phaseLetter" == "" ]]
then
        echo "var ReleasePhase: ReleasePhaseEnum = .Release" >> "${PROJECT_DIR}/version.swift"
        echo "let PhaseNumber = 0" >> "${PROJECT_DIR}/version.swift"
fi
